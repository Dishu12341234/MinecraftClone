...
void u_GraphicsPipeline::createGraphicsPipeline() {
  ...
  auto vertShaderCode =
      readFile("/home/divyansh/MinecraftClone/shaders/simple_shader_v.spv");
  auto fragShaderCode =
      readFile("/home/divyansh/MinecraftClone/shaders/simple_shader_f.spv");
  ...
  vertShaderStageInfo.stage =
      VK_SHADER_STAGE_VERTEX_BIT; // telling vulkan 'which stage must this
                                  // shader go'
  ...
  viewport.height =
      (float)swapChainExtent
          .height; // this swapchain is used as a framebuffer hensforth using it
                   // as the size for our viewport
...
void u_GraphicsPipeline::u_PassGraphicsPipelineCreateInfo(
    u_GraphicsPipelineCreateInfo pCreateInfo) {
...
VkShaderModule
u_GraphicsPipeline::createShaderModule(const std::vector<char> &code) {
...
// Telling vulkan how to pass data to the vertex shader once it's been uploaded
// into the GPU
VkVertexInputBindingDescription Vertex::getBindingDescription() {
  VkVertexInputBindingDescription bindingDescription{};
  // A vertex binding describes at which rate to load data from memory
  // throughout the verticies. It specifies the number of bytes between data
  // entries and whether to move to the next data entery after each vertex or
  // after each instance.

  bindingDescription.binding = 0; // binding index to zero
  bindingDescription.stride = sizeof(Vertex);
  bindingDescription.inputRate =
      VK_VERTEX_INPUT_RATE_VERTEX; // advance to the next input per vertex
...
Vertex::getAttributeDescriptions() {
  ...
  attributeDescriptions[0].location =
      0; // directly co-relates to 'layout(location = 0)'
