...
void HelloTriangleApplication::pickPhysicalDevice() {
  ...
  physicalDevice =
      VK_NULL_HANDLE; // this will be implicitly cleanup when VkInstance is
                      // destroid(no additoinal cleanup needed)
...
// Queue Family -> Conatains multiple queues for the GPU(Note: A queue family
// must be homogenous)
...
HelloTriangleApplication::findQueueFamilies(VkPhysicalDevice device) {
  ...
  vkGetPhysicalDeviceQueueFamilyProperties(
      ...
      nullptr); // grts all the queue families supported by the GPU
...
void HelloTriangleApplication::createLogicalDevice() {
  ...
  // Queue Family Stuff
  QueueFamilyIndices indices =
      findQueueFamilies(physicalDevice); // get the required queue families
  ...
  for (uint32_t queueFamily : uniqueQueueFamilies) // prepare each queue family
  {
    ...
    queueCreateInfos.push_back(
        queueCreateInfo); // push the queue info in the vector
  }
...
bool HelloTriangleApplication::isDeviceSuitable(VkPhysicalDevice device) {
  ...
  if (!indexingSupport.shaderSampledImageArrayNonUniformIndexing) {
    throw std::runtime_error(
        "Device does not support non-uniform texture indexing!");
  }
...
bool HelloTriangleApplication::checkDeviceExtensionSupport(
  ...
  vkEnumerateDeviceExtensionProperties(
      ...
      availableExtansions
          .data()); // all the required extensions are now retrived
  ...
  // if all the required extensions are supported then the 'requiredExtension'
  // vector is empty which returns true
...
// This function asks the GPU about the capablitie of the SwapChain that this
// GPU has
...
// this specifies how to presnet the images to the screen (vsync or mailbox or
// normal)
...
// MacOS has a fixed extent while other windowing systems have a varible extent
// which is clamped to the window bounds also High-DPI displays like Apple
// Liquid Retina displays have to be configure in a sepreate way
...
#else
  // Metal IE: the size is predifined and is not UINT 32 max. therefore, use the
...
// here we are using a staging buffer which is accisible by the CPU and then
// afterwards copying the contents of the staging buffer to the vertex buffer
