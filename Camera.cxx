...

{
  ...
  for (float t = 0.0f; t < maxDistanceCR; t += stepSize) {
    ...
    BlockCoordinates blockCoords{static_cast<int>(floor(point.x)),
...
{}
void Camera::updateUBO(UniformBufferObject &UBO, VkExtent2D &swapChainExtent,
                       Event &event) {
  ...
  double dx = (lastX - event.mouseX) * !locked;
  double dy = (lastY - event.mouseY) * !locked;
  ...
  UBO.proj = glm::perspective(
      glm::radians(90.0f),
      swapChainExtent.width / (float)swapChainExtent.height, 0.025f, 1000.0f);
